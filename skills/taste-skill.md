...
- **One palette row per project.** Choose ONE row from ui-ux-pro-max CSV data. Use EVERY color in that row: Primary, Secondary, Accent, Background, Foreground, Card, Card Foreground, Muted, Muted Foreground, Border, Destructive, Ring. Do not omit colors, do not invent extras.
- **Palette is authoritative.** The CSV row dictates saturation, contrast ratios, and color roles. Trust it.
- **THE LILA RULE:** The "AI Purple / Blue glow" aesthetic is discouraged as a default. No automatic purple button glows, no random neon gradients. Use neutral bases (Zinc / Slate / Stone) with high-contrast singular accents (Emerald, Electric Blue, Deep Rose, Burnt Orange, etc.).
- **Override:** if the brand or brief explicitly asks for purple / violet / lila, embrace it. But execute with intent: consistent palette, harmonised neutrals, restrained gradients. Not generic AI gradient slop.
- **One palette per project.** Do not fluctuate between warm and cool grays within the same project.
- **COLOR CONSISTENCY LOCK (mandatory):** Once a CSV palette row is chosen for a project, EVERY color in that row is locked. A warm-grey palette row does not have a cool-blue CTA. A rose-accented palette row does not have a teal status badge in the footer. Pick one palette row, lock EVERY value, audit every component before shipping.
...
* **Spec sheets specifically (the Marrow-cookware pattern).** A long product specification table with `border-b` on every row is the AI default for cookware / hardware / apparel / artisan-goods briefs. Banned. Concrete alternatives:
  - Grouped spec blocks with chunk headers and sparse dividers
...
* **NO real company names in testimonials.** NEVER mention real companies (Jira, Linear, Notion, Slack, Stripe, Vercel, etc.) in testimonial quotes — even as comparisons ("We tried Jira and Linear"). The agent always defaults to the same 5-6 tech companies. Use fictional product names or generic descriptions ("We tried three other tools before finding this one").
